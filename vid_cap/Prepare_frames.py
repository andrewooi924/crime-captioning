...
def main():
    ...
    allocate_videos("vid_cap/AnomalyClips/", "vid_cap/AnomalyFrames/")
    file_path = os.path.join("vid_cap/input", os.listdir("vid_cap/input")[0])
    os.remove(file_path)    
    for file in os.listdir("vid_cap/Eval_Res"):
        file_path = os.path.join("vid_cap/Eval_Res", file)
        os.remove(file_path)
