...
def demo_test(sess):
    ...
    for idx in range(0, tag_feat.shape[0]):
        ...
        sent_dict[idx] = [' '.join(sent_dict[idx])]
        sent_list.append(sent_dict[idx][0])
    with open("vid_cap/demo_test.log", 'w') as fo:
        for sent in sent_list:
            fo.write(sent+'\n')
...
if __name__ == "__main__":
    ...
    # tf.flags.DEFINE_string('name', 'UCF-CAP', 'name of model')
    # tf.flags.DEFINE_string('corpus', 'vid_cap/Files/ucfc-vd_Corpus.pkl', 'Path to corpus file')
    # tf.flags.DEFINE_string('ecores', 'vid_cap/ResnetFeatures/Scaled_ResNeXt.npy', 'Path to ECO-RES feature files')
    # tf.flags.DEFINE_string('tag', 'vid_cap/ucfcvd_e1000_tag_feats.npy', 'Path to Tag feature files')
    # tf.flags.DEFINE_string('ref', 'vid_cap/Files/ucfc-vd_ref.pkl', 'Path to reference files')
    # tf.flags.DEFINE_string('test', 'vid_cap/saves/UCF-CAP-best.ckpt', 'Path to the saved parameters')
    # tf.flags.DEFINE_string('demo', 'True', 'True or False: To test the model on individual samples')
    tf.flags.DEFINE_string('name', 'UCF-CAP', 'name of model')
    tf.flags.DEFINE_string('corpus', None, 'Path to corpus file')
    tf.flags.DEFINE_string('ecores', None, 'Path to ECO-RES feature files')
    tf.flags.DEFINE_string('tag', None, 'Path to Tag feature files')
    tf.flags.DEFINE_string('ref', None, 'Path to reference files')
    tf.flags.DEFINE_string('test', None, 'Path to the saved parameters')
    tf.flags.DEFINE_string('demo', None, 'True or False: To test the model on individual samples')
