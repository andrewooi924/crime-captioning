...
def generate_feat(inputx, model, out_feats, sess):
    ...
    print('Successfully generated features')

if __name__ == "__main__":
    global flags
    #Directory address to read the frames
    tf.app.flags.DEFINE_string('input', 'vid_cap/AnomalyFrames', 'input path')  
    #Address where npy file has to be saved 
    tf.app.flags.DEFINE_string('output', 'vid_cap/ResnetFeatures/ResNeXt', 'output path')  
    tf.app.flags.DEFINE_string('scaled_output', 'vid_cap/ResnetFeatures/Scaled_ResNeXt', 'scaled output path')
    flags = tf.app.flags.FLAGS
    videos = os.listdir(flags.input)
