...
class TagNet():
    def __init__(self):
        ...
        with self.graph.as_default():
            ...
            self.z = tf.placeholder(tf.float32, [None, n_z])
            self.rate = tf.placeholder(tf.float32, [])
            self.Wy1 = tf.get_variable('Wy1', [n_z, 512], tf.float32, tf.keras.initializers.glorot_normal())
            self.by1 = tf.get_variable('by1', [512], tf.float32, tf.keras.initializers.zeros())
            self.Wy2 = tf.get_variable('Wy2', [512, 512], tf.float32, tf.keras.initializers.glorot_normal())
            self.by2 = tf.get_variable('by2', [512], tf.float32,tf.keras.initializers.zeros())
            self.Wy3 = tf.get_variable('Wy3', [512, n_y], tf.float32, tf.keras.initializers.glorot_normal())
            self.by3 = tf.get_variable('by3', [n_y], tf.float32, tf.keras.initializers.zeros())

            z = tf.nn.dropout(self.z, self.rate)
            h = tf.nn.relu(tf.matmul(z, self.Wy1) + self.by1)
            h = tf.nn.dropout(h, self.rate)
